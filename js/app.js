// File: js/app.js
// Student: Nasserallah Abdelrahman Asad Salhi (12428281)
...
function buildUrl(endpoint, params = {}) {
  ...
  url.searchParams.set("stdid", STUDENT_ID);
...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    console.error(err);
    setStatus(err.message || "Error loading tasks", true);
  }
});
...
if (form) {
  form.addEventListener("submit", async function (event) {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success === false) throw new Error(data.message || "Add failed");

      // API غالبًا يرجع task جاهز
      const newTask = data.task || { id: data.id, title };
      renderTask(newTask);
      ...
      console.error(err);
      setStatus(err.message || "Error adding task", true);
    }
  });
}
...
function renderTask(task) {
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      const res = await fetch(buildUrl("delete.php", { id }));
      ...
      console.error(err);
      setStatus(err.message || "Error deleting task", true);
    }
  });
